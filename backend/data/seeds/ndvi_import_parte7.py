"""Import parte7 (Monteria) into Supabase - APPEND mode"""
...
async def main():
    ...
    await conn.execute("CREATE INDEX IF NOT EXISTS idx_indices_satelitales_ubicacion ON indices_satelitales USING GIST (ubicacion)")
    await conn.execute("CREATE INDEX IF NOT EXISTS idx_indices_satelitales_coords ON indices_satelitales (lat, lng)")

    await conn.close()
    print(f"\nListo. {kept} filas insertadas de {total} leidas (Monteria)")
