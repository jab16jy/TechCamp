"""Import PARTE 4 + 5 — Cartagena + Santa Marta-B NDVI into local Docker PostGIS"""
import asyncio
import csv
import math
...
FILES = [
    ("Cartagena", Path(r"D:\sentinel 2\nvdi\nvdi_parte4\nvdi_parte4final.csv"), 10, 0.1),
    ("Santa Marta-B", Path(r"D:\sentinel 2\nvdi\nvdi_parte5\nvdi_parte5final.csv"), 10, 0.1),
]
...
async def main():
    ...
    print(f"Registros antes: {result[0]['c']}")
    total_all = 0
    kept_all = 0
    for name, path, sample_every, ndvi_min in FILES:
        ...
        kept_all += kept
    result = await conn.fetch("SELECT COUNT(*) AS c FROM indices_satelitales")
    print(f"\nRegistros despues: {result[0]['c']}")
    await conn.close()
    print(f"\n=== COMPLETADO: {kept_all} nuevas filas ===")
