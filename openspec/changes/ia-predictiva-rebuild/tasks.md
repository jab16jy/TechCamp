...
- [x] 4.1 Reescribir `src/features/predictions/pages/IAPredictiva.jsx`: layout 6 filas BentoGrid con componentes nuevos, máquina de estados explícita, feature flag `VITE_USE_NEW_PREDICTION_HOOKS`. Capacidad: todas.
- [x] 4.2 Eliminar `RiskDetectionPanel/`, `MitigationSimulator/`, `SensorMicroGrid/`, `SensorStatusCard/` de `src/features/predictions/components/`. Capacidad: todas.
- [x] 4.3 Eliminar `usePlanRiegoActivo.jsx` y limpiar código muerto en `analysisService`. Capacidad: todas.
